...
testChangeRepeats = do
  ...
  checkResult (testName ++ "test1") $ repeats (config $ changeRepeats 4 testingBot1) == 4
  checkResult (testName ++ "test2") $ repeats (config $ changeRepeats 2 testingBot1) == 2
  checkResult (testName ++ "test3") $ repeats (config $ changeRepeats 0 testingBot1) == 0
...
testProcessUpdates = do
  ...
  checkResult (testName ++ "test2") $
    isNothing $ processUpdates 23 $ Right upds
